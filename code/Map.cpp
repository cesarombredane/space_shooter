...
Map::Map(int lvl) {
	if (!tileSetTexture.loadFromFile("files/graphics/tileset.png"))
		cerr << "erreur chargement de la tileset" << endl;
	else tileSet.setTexture(tileSetTexture);

	if (!backgroundTexture.loadFromFile("files/graphics/background.png"))
		cerr << "erreur chargement du BG" << endl;
	else background.setTexture(backgroundTexture);

	if (!buffer_death_enemy.loadFromFile("files/sons/death_enemy.wav")) {
...
void Map::load_map(int lvl) {
	...
	{
	case 1:
		maptxt.open("files/maps/map1.txt");
		break;
	case 2:
		maptxt.open("files/maps/map2.txt");
		break;
	case 3:
		maptxt.open("files/maps/map3.txt");
		break;
	default:
		break;
	}
...
void Map::test_mort_enemies(int volume) {
	for (int i = 0; i < enemy_map.size(); i++) {
		if (enemy_map[i].get_vie() <= 0) {
			...
			switch (difficulte) {
			case 1:
				if (enemy_map[i].get_type() == 0) add_point(50);
				else add_point(100);
				break;
			case 2:
				if (enemy_map[i].get_type() == 0) add_point(100);
				else add_point(200);
				break;
			case 3:
				if (enemy_map[i].get_type() == 0) add_point(250);
				else add_point(500);
				break;
			default:
				break;
